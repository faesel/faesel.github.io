...
export default function ShareButtons({ url, title, description, variant = 'inline' }: ShareButtonsProps) {
  const [copied, setCopied] = useState(false);

  const shareUrl = encodeURIComponent(url);
  const shareTitle = encodeURIComponent(title);
  const shareText = description ? encodeURIComponent(description) : shareTitle;
  ...
  const containerClass = isSidebar ? styles.floating : styles.container;

  return (
    <div className={containerClass}>
      {!isSidebar && <div className={styles.label}>Share this article</div>}
      ...
      {copied && <div className={styles.copyMessage}>Link copied!</div>}
    </div>
  );
}
